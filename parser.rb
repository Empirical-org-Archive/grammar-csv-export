...
class Parser
  ...
  def initialize
    ...
    @csv_arrays = [['Concept Level 1', 'CCSS Standard',	'Question UID',	'Answers', 'Prompt']]
    @questions_hash = JSON.parse(@file)
    @missing_fields = ["-Jzw0qjO5owyFPUAwDGK", "-Jzw0qjLNEeZAERlRksx", "-Jzw0qjO5owyFPUAwDGM", "-K2nXnevRx0EgXW07mzg"]
  end
    ...
    @questions_hash.each do |concept|
      unless @missing_fields.include?(concept.first)
        ...
        concept[1]["questions"].each do |question|
          ...
          @csv_arrays.push([concept_level_1, ccss, question_uid, answers, prompt])
        end
      end
    end
  end
